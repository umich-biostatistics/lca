...
where category is indexed by 1 through \eqn{\underset{l}{\max K_l}}; for variables where the number of categories is less than \eqn{\underset{l}{\max K_l}}, these cells will be set to NA. The parameters outputted by the lcra function currently are not user definable.
...
For more examples, run `?lcra` at the console, and scroll to Examples in the documentation file.
